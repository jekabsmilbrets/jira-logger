...
test('private health listener narrows its contract and releases its port', async () => {
  ...
  const duplicate = new HealthServer({
    ready: async () => {
    }
  }, address.port);
  ...
  try {
    ...
    assert.equal((await fetch(url + '/internal/ready', {
      method: 'POST'
    })).status, 404);
...
test('private health close drains an active readiness probe', {
  timeout: 5000
}, async () => {
  let enter, release;
  ...
  await server.start();
  const response = fetch(`http://127.0.0.1:${ server.address().port }/internal/ready`, {
    headers: {
      Connection: 'close'
    }
  });
