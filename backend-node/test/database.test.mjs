...
test('transactions preserve ordering, rollback and release with injected sessions', async () => {
  const calls = [];
  const session = { query: async sql => { calls.push(sql); return { rows: [], rowCount: 0 }; }, release: () => calls.push('release') };
  const database = new Database({ ...session, connect: async () => session, end: async () => calls.push('end') });
  assert.equal(await database.transaction(async client => { await client.query('write'); return 7; }), 7);
  assert.deepEqual(calls, ['BEGIN', 'write', 'COMMIT', 'release']);
  calls.length = 0;
  await assert.rejects(database.transaction(async () => { throw new Error('fixture'); }), /fixture/);
