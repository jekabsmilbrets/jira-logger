...
// Transitional exports until all callers receive application-owned dependencies.
export const db = createPool(config.database);
export const transaction = <T>(operation: (client: QueryExecutor) => Promise<T>): Promise<T> => new Database(db).transaction(operation);
