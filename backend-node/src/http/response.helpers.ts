...
import type { AcceptedMediaType, ProblemDetails } from '@http/http.types';
...
export function envelope(
  ...
  if (data && data !== '0' && !(Array.isArray(data) && data.length === 0)) {
    output.data = data;
  }
...
export function frameworkError(
  ...
  const accepted: AcceptedMediaType[] = (request.headers.accept ?? '').split(',').map(
    (
      ...
      const [type, ...parameters] = value.trim(
      ).split(';');
      const quality: string | undefined = parameters.find(
        (
          value,
        ) => value.trim(
        ).startsWith('q='));
  ...
  const known: string[] = ['text/html', 'application/xhtml+xml', 'text/plain', 'application/json', 'application/x-json', 'text/xml', 'application/xml', 'application/x-xml', 'application/problem+json'];
  const format: string | undefined = accepted.find(
    (
      value,
    ) => known.includes(value.type!,
    ))?.type;
  ...
  if (format === 'application/json' || format === 'application/x-json') {
    return reply.type('application/json').send(problem);
  }
