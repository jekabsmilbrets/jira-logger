...
import { FileLogStream } from '../logging/file-log-stream.js';
import { ApiError } from './api-error.js';
import { HealthServer } from './health-server.js';
import { CORS_METHODS, HSTS_POLICY, PUBLIC_HOST, READINESS_PATH } from './http.constants.js';
import type { Route } from './http.types.js';
import { envelope, frameworkError } from './response.helpers.js';
...
export class HttpServer {
  ...
  constructor(private readonly application: Application) {
    ...
    this.server = Fastify({
      logger: { redact: ['req.headers.authorization'], ...(config.logFile ? { stream: new FileLogStream(config.logFile) } : {}) },
    ...
    this.server.setErrorHandler((error, request, reply) => this.handleError(error, request, reply));
    this.server.all('/*', (request, reply) => this.dispatch(request, reply));
    this.server.addHook('onClose', async () => {
      try { await this.health.close(); }
      finally { await application.close(); }
    });
  }

  get instance(): FastifyInstance { return this.server; }
  ...
  async start(): Promise<FastifyInstance> {
    try {
      ...
      await this.server.listen({ host: PUBLIC_HOST, port: this.application.config.port });
      return this.server;
  ...
  private configureContent(): void {
    this.server.register(fastifyStatic, { root: this.application.config.assets, prefix: '/ng/', dotfiles: 'ignore' });
    this.server.removeAllContentTypeParsers();
    this.server.addContentTypeParser('*', { parseAs: 'string' }, (_request, value, done) => done(null, value));
  }

  private configureHeaders(): void {
    this.server.addHook('onSend', async (_request, reply, payload) => {
      if (this.application.config.tlsCertificate) reply.header('Strict-Transport-Security', HSTS_POLICY);
      if (String(reply.getHeader('content-type')).startsWith('application/json')) reply.header('content-type', 'application/json');
      return payload;
    });
    this.server.addHook('onRequest', (request, reply) => this.handleRequest(request, reply));
  }

  private handleError(error: unknown, request: FastifyRequest, reply: FastifyReply): FastifyReply {
  ...
  private async handleRequest(request: FastifyRequest, reply: FastifyReply): Promise<unknown> {
    const path = decodeURIComponent(new URL(request.url, 'http://localhost').pathname);
    if (path === READINESS_PATH || /\.php(?:\/|$)/.test(path)) return reply.code(404).send();
    const origin = request.headers.origin;
    if (!origin) return;
    const allowed = new RegExp(this.application.config.corsOrigin).test(origin);
  ...
  private async dispatch(request: FastifyRequest, reply: FastifyReply): Promise<unknown> {
