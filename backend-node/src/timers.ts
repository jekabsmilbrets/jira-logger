...
import { parseDate, sqlDate, DateCodec, type TimezoneProvider } from './dates.js';
import { timerView, type TaskResponse } from './projections.js';
...
export class TimersService {
  constructor(private readonly repository: TimersStore, private readonly tasks: Pick<TasksService, 'get' | 'show'>, private readonly timezone: TimezoneProvider, private readonly dates: DateCodec) {}
...
async save(input: Record<string, unknown>, taskId: string, id?: string): Promise<ReturnType<typeof timerView>> {
  ...
  try {
    const row = await this.repository.save({ id: id ?? randomUUID(), taskId,
      start: start ? sqlDate(start) : old?.start_time ?? null, end: end ? sqlDate(end) : old?.end_time ?? null,
      description: typeof input.description === 'string' ? input.description : old?.description ?? null }, Boolean(id));
    return timerView(row, zone);
  } catch { throw new ApiError(400, [`Can not ${id ? 'Update' : 'Create'} TimeLog`]); }
}
  async list(taskId: string): Promise<ReturnType<typeof timerView>[]> {
    const rows = await this.repository.forTask(taskId);
    if (!rows.length) throw new ApiError(404, ['TimeLogs not found']);
    const zone = await this.timezone.userTimezone();
    return rows.map(row => timerView(row, zone));
  }
  async show(taskId: string, id: string): Promise<ReturnType<typeof timerView>> {
    return timerView(await this.get(taskId, id, 'TimeLogs not found'), await this.timezone.userTimezone());
  }
