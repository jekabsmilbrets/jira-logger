...
import { record } from '../../shared/coercion.js';
import { JiraError } from './jira-error.js';
import { JIRA_TIMEOUT_MS } from './jira.constants.js';
import type { JiraSession } from './jira.types.js';

export class JiraHttpSession implements JiraSession {
  constructor(private readonly dispatcher: Dispatcher, private readonly host: string, private readonly token: string) { }

  async request(path: string, payload: unknown, method = 'POST'): Promise<Record<string, unknown>> {
    const url = this.host.replace(/\/$/, '') + '/rest/api/2' + path;
    let response;
    const started = Date.now();
    try {
      response = await fetch(url, {
        method, body: JSON.stringify(payload), dispatcher: this.dispatcher, signal: AbortSignal.timeout(JIRA_TIMEOUT_MS), headers: {
          Accept: '*/*', 'Content-Type': 'application/json', 'X-Atlassian-Token': 'no-check', 'X-ExperimentalApi': 'opt-in', Authorization: `Bearer ${this.token}`,
        }
      });
    } catch (error) {
