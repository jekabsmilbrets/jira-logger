...
test('Jira sessions retain their configuration and share only the injected transport', async () => {
  ...
  const client = new JiraClient({
    jiraConfiguration: async () => ({
      'jira.enabled': 'true',
      'jira.host': 'https://jira.invalid/context/',
      'jira.personal-access-token': token
    })
  }, dispatcher);
  ...
  try {
    ...
    for (const [session, credential, id] of [[first, 'first-fixture-token', '1'], [second, token, '2']]) {
      dispatcher.get('https://jira.invalid').intercept({
        path: '/context/rest/api/2/issue/DEMO-1/worklog',
        method: 'POST',
        headers: {
      ...
      assert.deepEqual(await session.request('/issue/DEMO-1/worklog', {
        timeSpentSeconds: 60
      }), {
        id
      });
