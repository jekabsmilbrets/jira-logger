  ...
  async seed(name: 'setting' | 'tag', entries: readonly (readonly [string, string | undefined])[], unload: boolean): Promise<void> {
    await this.database.transaction(async client => {
      ...
      for (const [key, value] of entries) {
        ...
        if (name === 'setting') {
          if (value === undefined) throw new Error('Missing seed value');
          values.push(value);
        }
  ...
  async createDatabase(): Promise<void> {
    ...
    try {
      ...
      if (!(await admin.query('SELECT 1 FROM pg_database WHERE datname=$1', [name])).rowCount) {
        ...
        catch (error) { if (errorCode(error) !== '42P04') throw error; }
      }
    } finally { await admin.end(); }
  }
  async audit(): Promise<AuditResult> {
    const duplicates = await this.database.query<AuditResult['duplicates'][number]>('SELECT task_id,start_time,work_log_id,COUNT(*) AS duplicate_count FROM jira_work_log GROUP BY task_id,start_time,work_log_id HAVING COUNT(*)>1 ORDER BY duplicate_count DESC,task_id ASC');
    const invalid = await this.database.query<AuditResult['invalidTimeLogs'][number]>('SELECT id,task_id,start_time,end_time FROM time_log WHERE end_time IS NOT NULL AND end_time<=start_time ORDER BY task_id ASC,start_time ASC');
