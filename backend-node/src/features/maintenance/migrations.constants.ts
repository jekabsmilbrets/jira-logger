import type { Migration } from '@features/maintenance/maintenance.types';


export const migrationLock: number = 20221216190644;

const tables: string[] = ['jira_work_log', 'setting', 'tag', 'task', 'time_log'];

const timestamps: string = 'created_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL, updated_at TIMESTAMP(0) WITHOUT TIME ZONE NOT NULL';

export const migrations: Migration[] = [
  {
    version: 'DoctrineMigrations\\Version20221216190644',
    sql: [
      `CREATE TABLE jira_work_log
       (
           id                 UUID         NOT NULL,
           task_id            UUID         NOT NULL,
           work_log_id        VARCHAR(255) NOT NULL,
      ...
      ...tables.map(
        (
          table,
        ) => `COMMENT ON COLUMN ${ table }.id IS '(DC2Type:uuid)'`,
      ),
      ...['jira_work_log.task_id', 'time_log.task_id', 'tag_task.tag_id', 'tag_task.task_id'].map(
        (
          column,
        ) => `COMMENT ON COLUMN ${ column } IS '(DC2Type:uuid)'`,
      ),
  ...
  {
    version: 'DoctrineMigrations\\Version20250606180000',
    sql: tables.map(
      (
        table,
      ) => {
        const columns: string[] = table === 'time_log' ? ['start_time', 'end_time', 'created_at', 'updated_at'] : ['created_at', 'updated_at'];

        return `ALTER TABLE ${ table } ` + columns.map(
          (
            column,
          ) => `ALTER COLUMN ${ column } TYPE TIMESTAMPTZ USING ${ column } AT TIME ZONE 'Europe/Riga'`,
        ).join(', ');
      })
  }
];
