...
export class TimersRepository {
  ...
  public async stopAll(): Promise<void> {
    await this.database.query('UPDATE time_log SET end_time=NOW() WHERE end_time IS NULL');
  }
  ...
  public async latestRunning(
    taskId: string,
  ): Promise<string | undefined> {
    return (await this.database.query<{
      id: string
    }>('SELECT id FROM time_log WHERE task_id=$1 AND end_time IS NULL ORDER BY start_time DESC,created_at DESC LIMIT 1', [taskId])).rows[0]?.id;
  }
  ...
  public async overlapping(
    start: string | null,
    end: string | null,
  ): Promise<Pick<TimerRow, 'start_time' | 'end_time'>[]> {
