...
import { redacted }           from '@features/settings/settings.constants';
...
function disclose(
  ...
  return {
    id: row.id,
    name: row.name,
    value: /token|password|secret|key/i.test(row.name) ? redacted : row.value
  };
}
...
export class SettingsService {
  ...
  private async get(
    ...
    const row: SettingRow | undefined = await this.repository.find(id);
  ...
  public async save(
    ...
    if (input.value === redacted) {
      throw new ApiError(400, ['Redacted setting values cannot be stored.']);
    }
    ...
    try {
      if (old && old.name === input.name && old.value === input.value) {
        return disclose(old);
      }

      return disclose(await this.repository.save({
        id: id ?? randomUUID(),
        name: input.name,
        value: input.value
      }, Boolean(id)));
  ...
  public async list(): Promise<SettingRow[]> {
    const rows: SettingRow[] = await this.repository.list();
  ...
  public async show(
    ...
    return disclose(await this.get(id));
  }
  ...
  public async delete(
    ...
    try {
      ...
      throw new ApiError(400, ['Can not Delete Setting']);
    }
  }
}
