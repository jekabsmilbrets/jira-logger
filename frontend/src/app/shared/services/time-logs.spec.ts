...
describe('Shared Services time-logs', () => {
  let service: TimeLogs;
  ...
  beforeEach(async () => {
    ...
    service = TestBed.inject(TimeLogs);
    vi.clearAllMocks();
  });
  ...
  it('registers its loading state on initialization', () => {
    ...
    expect(service.loaderStateService.addLoader).toHaveBeenCalledWith(service.isLoading, 'TimeLogs');
  });
});
