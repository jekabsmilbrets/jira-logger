...
const testId: string = '13d0305b-d6f2-4344-bb30-d5100d56f568';
const testName: string = 'JIRA-LOGGER-0001';
...
describe('SettingsService', () => {
  ...
  it('should call method "list" and "addLoader" on LoaderStateService on method "init" call', () => {
  ...
  it('should call method "list" on calling method "reloadList"', () => {
  ...
  it('should return cached Settings on calling method "reloadList with arg "true"', () => {
  ...
  it('should find Setting in Setting list on calling method "findSetting"', () => {
    const adaptedSettings: Setting[] = adaptSettings(testSettings);
  ...
  it('should throw error on trying to find nonexistent Setting on calling method "findSetting"', () => {
    const adaptedSettings: Setting[] = adaptSettings(testSettings);
  ...
  it(`should update isLoadingSubject, call "errorDialogService.openDialog" and
  ...
  describe('fetching Settings from API via method "list"', () => {
    afterEach(() => {
      httpMock.verify();
    });

    it('should try to fetch all Settings with API GET request', () => {
    ...
    it(`should return empty array when try to fetch all Settings with API GET request but receives 404`, () => {
      const url = `${ environment['apiHost'] }${ environment['apiBase'] }/${ (service as any).basePath }`;
      ...
      const request = httpMock.expectOne(url);
      expect(request.request.method).toEqual('GET');
      request.flush(errorMsg, {
        status: 404,
        statusText: 'Not Found',
      });
    });

    it(`should throw error when try to fetch all Settings with API GET request but receives error`, () => {
      const url = `${ environment['apiHost'] }${ environment['apiBase'] }/${ (service as any).basePath }`;
  ...
  describe('creating Setting at API via method "create"', () => {
    ...
    it('should return newly created Setting with API POST request', () => {
      const url = `${ environment['apiHost'] }${ environment['apiBase'] }/${ (service as any).basePath }`;
      ...
      service
        .create(testCreateSetting, skipReload)
        .pipe(take(1))
        .subscribe((response: Setting) => {
          expect(waitForTurnSpy).toHaveBeenCalled();
          expect(isLoadingSubjectSpy).toHaveBeenCalled();
    ...
    it(`should throw error when try to create Setting at API on calling method "create" but receives error`, () => {
      const url = `${ environment['apiHost'] }${ environment['apiBase'] }/${ (service as any).basePath }`;
      const errorMsg = 'Internal Error';

      service
        .create(testCreateSetting)
        .pipe(take(1))
        .subscribe({
  ...
  describe('updating Setting at API via method "update"', () => {
    ...
    it('should return updated Setting with API PATCH request', () => {
      ...
      service
        .update(testUpdateSetting, skipReload)
        .pipe(take(1))
        .subscribe((response: Setting) => {
          expect(waitForTurnSpy).toHaveBeenCalled();
          expect(isLoadingSubjectSpy).toHaveBeenCalled();
    ...
    it(`should throw error when try to update Setting at API on calling method "update" but receives error`, () => {
      const url = `${ environment['apiHost'] }${ environment['apiBase'] }/${ (service as any).basePath }/${ testId }`;
      const errorMsg = 'Internal Error';

      service
        .update(testUpdateSetting)
        .pipe(take(1))
        .subscribe({
          next: () => fail('should have failed'),
          error: (error: HttpErrorResponse) => {
  ...
  describe('deleting Setting from API via method "delete"', () => {
    ...
    it('should delete Setting with API DELETE request', () => {
      const url = `${ environment['apiHost'] }${ environment['apiBase'] }/${ (service as any).basePath }/${ testId }`;
      const adaptedSettings: Setting[] = adaptSettings(testSettings);
      const listSpy = spyOn(service, 'list').and.returnValue(of(adaptedSettings));

      service
        .delete(testDeleteSetting)
        .pipe(take(1))
        .subscribe(() => {
          expect(waitForTurnSpy).toHaveBeenCalled();
          expect(isLoadingSubjectSpy).toHaveBeenCalled();
    ...
    it(`should throw error when try to delete Setting at API on calling method "delete" but receives error`, () => {
      const url = `${ environment['apiHost'] }${ environment['apiBase'] }/${ (service as any).basePath }/${ testId }`;
      const errorMsg = 'Internal Error';
