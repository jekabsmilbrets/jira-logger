...
describe('Report Service Report', () => {
  ...
  let reportColumnsService: ReportColumns;
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        Report,
        { provide: TaskQuery, useValue: taskQueryService },
    ...
    reportColumnsService = TestBed.inject(ReportColumns);
  });
  ...
  it('applies fallback settings when persisted settings are missing', async () => {
    ...
    TestBed.configureTestingModule({
      providers: [
        Report,
        { provide: TaskQuery, useValue: localTaskQueryService },
        { provide: Storage, useValue: localStorageService },
        { provide: Settings, useValue: { settings: signal<Setting[]>([]).asReadonly() } },
        { provide: Timezone, useValue: { timezone: 'UTC' } },
        {
          provide: Tags,
    ...
    const localService = TestBed.inject(Report);
  ...
  it('handles persisted settings without tags list', async () => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        { provide: Settings, useValue: { settings: signal<Setting[]>([]).asReadonly() } },
        { provide: Timezone, useValue: { timezone: 'UTC' } },
        { provide: Tags, useValue: { tags: localTagsState.asReadonly() } },
      ],
    });
  ...
  it('does not persist default state before startup hydration finishes', async () => {
    ...
    TestBed.configureTestingModule({
      providers: [
        Report,
        { provide: TaskQuery, useValue: { query: vi.fn(() => of([])) } },
        { provide: Storage, useValue: localStorageService },
        { provide: Settings, useValue: { settings: signal<Setting[]>([]).asReadonly() } },
        { provide: Timezone, useValue: { timezone: 'UTC' } },
        {
  ...
  it('applies report route params inside the report module', () => {
    ...
    expect(service.settingsControlsState().date?.getTime()).toBe(
      TestBed.inject(ReportDateCalendar).parseRouteDate('2026-05-30')?.getTime(),
    );
  });
  ...
  it('applies today for date mode route without date param', () => {
    ...
    expect(service.settingsControlsState().reportMode).toBe(ReportMode.date);
    expect(service.settingsControlsState().date?.getTime()).toBe(
      TestBed.inject(ReportDateCalendar).todayReportDate().getTime(),
    );
  });
  ...
  it('keeps route mode and date when delayed persisted settings hydrate', () => {
    ...
    expect(localService.settingsControlsState().date?.getTime()).toBe(
      TestBed.inject(ReportDateCalendar).parseRouteDate('2021-01-01')?.getTime(),
    );
  ...
  it('persists settings changes through Storage.create', async () => {
    applyTags(service, [{ id: 'tag-1', name: 'Backend' } as Tag]);
    applyHideUnreportedTasks(service, true);
    applyShowWeekends(service, true);
    applyReportMode(service, ReportMode.total);
